...
save.image("results_10.RData")
write.table(stored_move_params, file = "move_param_10.txt", row.names = F)
write.table(c(speed_param_count, speed_param_count / (num_iterations / num_extra_path_updates),
              refined_path_count, refined_path_count / num_iterations), file = "acceptance_10.txt", row.names = F)
...
write.table(samp_bearings, "bearings_10.txt", row.names = F)
write.table(samp_steps, "steps_10.txt", row.names = F)
write.table(samp_loc, "loc_10.txt", row.names = F)
