...
refined_path <- InitialPath(obs, time_scale = 1, error = FALSE, corr_error = FALSE)
...
write.table(list(times = refined_path$times,
                 ...
                 X = refined_path$X,
                 Y = refined_path$Y),
            "initial_path_one.txt", row.names = F)
...
save.image("results_one.RData")
write.table(stored_move_params, file = "move_param_one.txt", row.names = F)
write.table(c(speed_param_count, speed_param_count / (num_iterations / num_extra_path_updates),
              refined_path_count, refined_path_count / num_iterations), file = "acceptance_one.txt", row.names = F)
...
write.table(samp_bearings, "bearings_one.txt", row.names = F)
write.table(samp_steps, "steps_one.txt", row.names = F)
write.table(samp_loc, "loc_one.txt", row.names = F)
