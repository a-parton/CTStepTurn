...
save.image("results_45.RData")
write.table(stored_move_params, file = "move_param_45.txt", row.names = F)
write.table(c(speed_param_count, speed_param_count / (num_iterations / num_extra_path_updates),
              refined_path_count, refined_path_count / num_iterations), file = "acceptance_45.txt", row.names = F)
...
write.table(samp_bearings, "bearings_45.txt", row.names = F)
write.table(samp_steps, "steps_45.txt", row.names = F)
write.table(samp_loc, "loc_45.txt", row.names = F)
